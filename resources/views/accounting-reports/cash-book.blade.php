...
@section('content')
        ...
        <div class="content content-full">
            <div class="d-flex flex-column flex-sm-row justify-content-sm-between align-items-sm-center py-0">
                <div class="flex-grow-1 text-center text-sm-start">
                    <h5 class="h5 text-main fw-bold mb-1">Cash Book</h5>
                    <h2 class="fs-base lh-base fw-medium text-muted mb-0">Cash and bank transactions</h2>
                </div>
                <nav class="flex-shrink-0 mt-3 mt-sm-0 ms-sm-3" aria-label="breadcrumb">
                    <ol class="breadcrumb breadcrumb-alt">
                        ...
                        <li class="breadcrumb-item" aria-current="page">Cash Book</li>
                    </ol>
                </nav>
            </div>
        </div>
    ...
    <div class="content1 p-2 rounded-0">
        <div class="block block-rounded shadow-sm py-5 px-2 rounded-0">
                        ...
                        <select class="form-control" id="ledger_id" name="ledger_id">
                            ...
                            @foreach ($ledgers as $ledger)
                                <option value="{{ $ledger->id }}"
                                    {{ $selectedLedgerId == $ledger->id ? 'selected' : '' }}>{{ $ledger->name }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="col-md-3">
            ...
            @else
                <p class="text-center">Please select a cash or bank ledger to view transactions.</p>
            @endif
            @if ($ledger)
                <div class="text-end mt-3">
                    <button class="btn btn-alt-primary"
                        onclick="alert('Export to PDF functionality to be implemented.')">Export to PDF</button>
                    <button class="btn btn-alt-primary ms-2"
                        onclick="alert('Export to Excel functionality to be implemented.')">Export to Excel</button>
                </div>
            @endif
        </div>
    </div>
