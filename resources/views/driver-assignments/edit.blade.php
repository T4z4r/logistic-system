    ...
    <div class="content content-full">
      <div class="d-flex flex-column flex-sm-row justify-content-sm-between align-items-sm-center py-2">
        <div class="flex-grow-1">
          <h1 class="h3 fw-bold mb-1">Edit Driver Assignment</h1>
          <h2 class="fs-base lh-base fw-medium text-muted mb-0">Update driver assignment details</h2>
        </div>
  ...
  <div class="content">
    ...
    <div class="block block-rounded">
      <div class="block-header block-header-default">
        <h3 class="block-title">Edit Driver Assignment Form</h3>
      </div>
      <div class="block-content">
        @if (isset($assignment) && $assignment->id)
          <form action="{{ route('driver-assignments.update', $assignment->id) }}" method="POST">
            @csrf
            @method('PUT')
            <div class="mb-4">
              ...
              <select name="driver_id" id="driver_id" class="form-control @error('driver_id') is-invalid @enderror">
                ...
                @foreach ($drivers as $driver)
                  <option value="{{ $driver->id }}" {{ old('driver_id', $assignment->driver_id) == $driver->id ? 'selected' : '' }}>{{ $driver->name }}</option>
                @endforeach
              </select>
            ...
            <div class="mb-4">
              ...
              <select name="truck_id" id="truck_id" class="form-control @error('truck_id') is-invalid @enderror">
                ...
                @foreach ($trucks as $truck)
                  <option value="{{ $truck->id }}" {{ old('truck_id', $assignment->truck_id) == $truck->id ? 'selected' : '' }}>{{ $truck->license_plate }}</option>
                @endforeach
              </select>
        ...
        @else
          <div class="alert alert-danger" role="alert">Driver assignment not found.</div>
        @endif
      </div>
    </div>
