...
@section('content')
        ...
        <div class="block-header block-header-default">
            ...
            <div class="block-options">
                <button type="button" class="btn btn-alt-primary  btn-sm" data-bs-toggle="modal"
                    data-bs-target="#createSalesVoucherModal">
                            ...
                            <tr>
                                ...
                                <td>
                                    <button type="button" class="btn btn-sm btn-alt-primary" data-bs-toggle="modal"
                                        data-bs-target="#editSalesVoucherModal{{ $voucher->id }}">
                                        <i class="fa fa-edit"></i> Edit
                                    </button>
                                    <form action="{{ route('vouchers.sales.destroy', $voucher->id) }}" method="POST"
    ...
    <div class="modal fade" id="createSalesVoucherModal" tabindex="-1" aria-labelledby="createSalesVoucherModalLabel"
        aria-hidden="true">
                ...
                <div class="modal-body">
                    <form action="{{ route('vouchers.sales.store') }}" method="POST" id="createSalesVoucherForm">
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-control @error('sales_ledger_id') is-invalid @enderror"
                                id="create_sales_ledger_id" name="sales_ledger_id">
                                <option value="">Select Ledger</option>
                                @foreach ($ledgers as $ledger)
                                    <option value="{{ $ledger->id }}"
                                        {{ old('sales_ledger_id') == $ledger->id ? 'selected' : '' }}>{{ $ledger->name }}
                                    </option>
                                @endforeach
                            </select>
    ...
    @foreach ($vouchers as $voucher)
        <div class="modal fade" id="editSalesVoucherModal{{ $voucher->id }}" tabindex="-1"
            aria-labelledby="editSalesVoucherModalLabel{{ $voucher->id }}" aria-hidden="true">
            <div class="modal-dialog modal-lg">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="editSalesVoucherModalLabel{{ $voucher->id }}">Edit Sales Voucher
                        </h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <div class="modal-body">
                        <form action="{{ route('vouchers.sales.update', $voucher->id) }}" method="POST"
                            id="editSalesVoucherForm{{ $voucher->id }}">
                            @csrf
                            @method('PUT')
                            <div class="mb-3">
                            ...
                            <div class="mb-3">
                                ...
                                <select class="form-control @error('party_ledger_id') is-invalid @enderror"
                                    ...
                                    @foreach ($ledgers as $ledger)
                                        <option value="{{ $ledger->id }}"
                                            {{ old('party_ledger_id', $voucher->entries->where('type', 'debit')->first()->ledger_id) == $ledger->id ? 'selected' : '' }}>
                                            {{ $ledger->name }}</option>
                                    @endforeach
                                </select>
                                @error('party_ledger_id')
                            ...
                            <div class="mb-3">
                                <label for="edit_sales_ledger_id_{{ $voucher->id }}" class="form-label">Sales Ledger
                                    (Credit)</label>
                                <select class="form-control @error('sales_ledger_id') is-invalid @enderror"
                                    id="edit_sales_ledger_id_{{ $voucher->id }}" name="sales_ledger_id">
                                    <option value="">Select Ledger</option>
                                    @foreach ($ledgers as $ledger)
                                        <option value="{{ $ledger->id }}"
                                            {{ old('sales_ledger_id', $voucher->entries->where('type', 'credit')->first()->ledger_id) == $ledger->id ? 'selected' : '' }}>
                                            {{ $ledger->name }}</option>
                                    @endforeach
                                </select>
                                @error('sales_ledger_id')
                            ...
                            <div class="mb-3">
                                ...
                                <input type="number" step="0.01"
                                    ...
                                    value="{{ old('quantity', abs($voucher->stockEntries->first()->quantity)) }}">
                                @error('quantity')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                    ...
                    <div class="modal-footer">
                        ...
                        <button type="submit" form="editSalesVoucherForm{{ $voucher->id }}"
