...
@section('content')
  ...
  <div class="content1 p-2 rounded-0">
    <div class="block block-rounded shadow-sm py-5 rounded-0">
      ...
      <div class="mb-4 text-center">
        <button type="button" class="btn btn-alt-primary px-4 py-2" data-bs-toggle="modal" data-bs-target="#createReceiptVoucherModal">
          ...
          <tbody>
            @forelse ($vouchers as $voucher)
              <tr>
                ...
                <td>
                  <button type="button" class="btn btn-sm btn-alt-primary" data-bs-toggle="modal" data-bs-target="#editReceiptVoucherModal{{ $voucher->id }}">
                    <i class="fa fa-edit"></i> Edit
                  </button>
                  <form action="{{ route('vouchers.receipt.destroy', $voucher->id) }}" method="POST" style="display:inline;">
                    @csrf
                    @method('DELETE')
            ...
            @empty
              <tr>
                <td colspan="6" class="text-center">No receipt vouchers found.</td>
              </tr>
            @endforelse
          </tbody>
        </table>
  ...
  <div class="modal fade" id="createReceiptVoucherModal" tabindex="-1" aria-labelledby="createReceiptVoucherModalLabel" aria-hidden="true">
  ...
  @foreach ($vouchers as $voucher)
    <div class="modal fade" id="editReceiptVoucherModal{{ $voucher->id }}" tabindex="-1" aria-labelledby="editReceiptVoucherModalLabel{{ $voucher->id }}" aria-hidden="true">
      <div class="modal-dialog">
        <div class="modal-content">
          <div class="modal-header">
            <h5 class="modal-title" id="editReceiptVoucherModalLabel{{ $voucher->id }}">Edit Receipt Voucher</h5>
            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
          </div>
          <div class="modal-body">
            <form action="{{ route('vouchers.receipt.update', $voucher->id) }}" method="POST" id="editReceiptVoucherForm{{ $voucher->id }}">
              @csrf
              @method('PUT')
              <div class="mb-3">
              ...
              <div class="mb-3">
                <label for="edit_cash_bank_ledger_id_{{ $voucher->id }}" class="form-label">Cash/Bank Ledger (Debit)</label>
                <select class="form-control @error('cash_bank_ledger_id') is-invalid @enderror" id="edit_cash_bank_ledger_id_{{ $voucher->id }}" name="cash_bank_ledger_id">
                  <option value="">Select Ledger</option>
                  @foreach ($ledgers as $ledger)
                    <option value="{{ $ledger->id }}" {{ old('cash_bank_ledger_id', $voucher->entries->where('type', 'debit')->first()->ledger_id) == $ledger->id ? 'selected' : '' }}>{{ $ledger->name }}</option>
                  @endforeach
                </select>
                @error('cash_bank_ledger_id')
                  <div class="invalid-feedback">{{ $message }}</div>
          ...
          <div class="modal-footer">
            ...
            <button type="submit" form="editReceiptVoucherForm{{ $voucher->id }}" class="btn btn-alt-primary">
