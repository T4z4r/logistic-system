...
@section('content')
  ...
  <div class="content1 p-2 rounded-0">
    <div class="block block-rounded shadow-sm py-5 rounded-0">
      ...
      <div class="mb-4 text-center">
        <button type="button" class="btn btn-alt-primary px-4 py-2" data-bs-toggle="modal" data-bs-target="#createPurchaseVoucherModal">
          ...
          <tbody>
            @forelse ($vouchers as $voucher)
              <tr>
                ...
                <td>
                  <button type="button" class="btn btn-sm btn-alt-primary" data-bs-toggle="modal" data-bs-target="#editPurchaseVoucherModal{{ $voucher->id }}">
                    <i class="fa fa-edit"></i> Edit
                  </button>
                  <form action="{{ route('vouchers.purchase.destroy', $voucher->id) }}" method="POST" style="display:inline;">
                    @csrf
                    @method('DELETE')
            ...
            @empty
              <tr>
                <td colspan="7" class="text-center">No purchase vouchers found.</td>
              </tr>
            @endforelse
          </tbody>
        </table>
  ...
  <div class="modal fade" id="createPurchaseVoucherModal" tabindex="-1" aria-labelledby="createPurchaseVoucherModalLabel" aria-hidden="true">
        ...
        <div class="modal-body">
          <form action="{{ route('vouchers.purchase.store') }}" method="POST" id="createPurchaseVoucherForm">
            ...
            <div class="mb-3">
              ...
              <select class="form-control @error('purchase_ledger_id') is-invalid @enderror" id="create_purchase_ledger_id" name="purchase_ledger_id">
                <option value="">Select Ledger</option>
                @foreach ($ledgers as $ledger)
                  <option value="{{ $ledger->id }}" {{ old('purchase_ledger_id') == $ledger->id ? 'selected' : '' }}>{{ $ledger->name }}</option>
                @endforeach
              </select>
  ...
  @foreach ($vouchers as $voucher)
    <div class="modal fade" id="editPurchaseVoucherModal{{ $voucher->id }}" tabindex="-1" aria-labelledby="editPurchaseVoucherModalLabel{{ $voucher->id }}" aria-hidden="true">
      <div class="modal-dialog modal-lg">
        <div class="modal-content">
          <div class="modal-header">
            <h5 class="modal-title" id="editPurchaseVoucherModalLabel{{ $voucher->id }}">Edit Purchase Voucher</h5>
            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
          </div>
          <div class="modal-body">
            <form action="{{ route('vouchers.purchase.update', $voucher->id) }}" method="POST" id="editPurchaseVoucherForm{{ $voucher->id }}">
              @csrf
              @method('PUT')
              <div class="mb-3">
              ...
              <div class="mb-3">
                <label for="edit_purchase_ledger_id_{{ $voucher->id }}" class="form-label">Purchase Ledger (Debit)</label>
                <select class="form-control @error('purchase_ledger_id') is-invalid @enderror" id="edit_purchase_ledger_id_{{ $voucher->id }}" name="purchase_ledger_id">
                  <option value="">Select Ledger</option>
                  @foreach ($ledgers as $ledger)
                    <option value="{{ $ledger->id }}" {{ old('purchase_ledger_id', $voucher->entries->where('type', 'debit')->first()->ledger_id) == $ledger->id ? 'selected' : '' }}>{{ $ledger->name }}</option>
                  @endforeach
                </select>
                @error('purchase_ledger_id')
                  <div class="invalid-feedback">{{ $message }}</div>
              ...
              <div class="mb-3">
                <label for="edit_party_ledger_id_{{ $voucher->id }}" class="form-label">Party Ledger (Credit)</label>
                <select class="form-control @error('party_ledger_id') is-invalid @enderror" id="edit_party_ledger_id_{{ $voucher->id }}" name="party_ledger_id">
                  <option value="">Select Ledger</option>
                  @foreach ($ledgers as $ledger)
                    <option value="{{ $ledger->id }}" {{ old('party_ledger_id', $voucher->entries->where('type', 'credit')->first()->ledger_id) == $ledger->id ? 'selected' : '' }}>{{ $ledger->name }}</option>
                  @endforeach
                </select>
                @error('party_ledger_id')
                  <div class="invalid-feedback">{{ $message }}</div>
              ...
              <div class="mb-3">
                ...
                <input type="number" step="0.01" class="form-control @error('quantity') is-invalid @enderror" id="edit_quantity_{{ $voucher->id }}" name="quantity" value="{{ old('quantity', $voucher->stockEntries->first()->quantity) }}">
                @error('quantity')
                  <div class="invalid-feedback">{{ $message }}</div>
                @enderror
              </div>
          ...
          <div class="modal-footer">
            ...
            <button type="submit" form="editPurchaseVoucherForm{{ $voucher->id }}" class="btn btn-alt-primary">
