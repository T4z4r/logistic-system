...
@section('content')
        ...
        <div class="block-header block-header-default">
            ...
            <div class="block-options">
                <button type="button" class="btn btn-alt-primary btn-sm" data-bs-toggle="modal"
                    data-bs-target="#createPaymentVoucherModal">
                            ...
                            <tr>
                                ...
                                <td>
                                    <button type="button" class="btn btn-sm btn-alt-primary" data-bs-toggle="modal"
                                        data-bs-target="#editPaymentVoucherModal{{ $voucher->id }}">
                                        <i class="fa fa-edit"></i> 
                                    </button>
                                    <form action="{{ route('vouchers.payment.destroy', $voucher->id) }}" method="POST"
    ...
    <div class="modal fade" id="createPaymentVoucherModal" tabindex="-1" aria-labelledby="createPaymentVoucherModalLabel"
        aria-hidden="true">
    ...
    @foreach ($vouchers as $voucher)
        <div class="modal fade" id="editPaymentVoucherModal{{ $voucher->id }}" tabindex="-1"
            aria-labelledby="editPaymentVoucherModalLabel{{ $voucher->id }}" aria-hidden="true">
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="editPaymentVoucherModalLabel{{ $voucher->id }}">Edit Payment Voucher
                        </h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <div class="modal-body">
                        <form action="{{ route('vouchers.payment.update', $voucher->id) }}" method="POST"
                            id="editPaymentVoucherForm{{ $voucher->id }}">
                            @csrf
                            @method('PUT')
                            <div class="mb-3">
                            ...
                            <div class="mb-3">
                                ...
                                <select class="form-control @error('cash_bank_ledger_id') is-invalid @enderror"
                                    ...
                                    @foreach ($ledgers as $ledger)
                                        <option value="{{ $ledger->id }}"
                                            {{ old('cash_bank_ledger_id', $voucher->entries->where('type', 'credit')->first()->ledger_id) == $ledger->id ? 'selected' : '' }}>
                                            {{ $ledger->name }}</option>
                                    @endforeach
                                </select>
                                @error('cash_bank_ledger_id')
                    ...
                    <div class="modal-footer">
                        ...
                        <button type="submit" form="editPaymentVoucherForm{{ $voucher->id }}"
