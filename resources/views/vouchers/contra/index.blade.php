...
@section('content')
  ...
  <div class="content1 p-2 rounded-0">
    <div class="block block-rounded shadow-sm py-5 rounded-0">
      ...
      <div class="mb-4 text-center">
        <button type="button" class="btn btn-alt-primary px-4 py-2" data-bs-toggle="modal" data-bs-target="#createContraVoucherModal">
          ...
          <tbody>
            @forelse ($vouchers as $voucher)
              <tr>
                ...
                <td>
                  <button type="button" class="btn btn-sm btn-alt-primary" data-bs-toggle="modal" data-bs-target="#editContraVoucherModal{{ $voucher->id }}">
                    <i class="fa fa-edit"></i> Edit
                  </button>
                  <form action="{{ route('vouchers.contra.destroy', $voucher->id) }}" method="POST" style="display:inline;">
                    @csrf
                    @method('DELETE')
            ...
            @empty
              <tr>
                <td colspan="6" class="text-center">No contra vouchers found.</td>
              </tr>
            @endforelse
          </tbody>
        </table>
  ...
  <div class="modal fade" id="createContraVoucherModal" tabindex="-1" aria-labelledby="createContraVoucherModalLabel" aria-hidden="true">
        ...
        <div class="modal-body">
          <form action="{{ route('vouchers.contra.store') }}" method="POST" id="createContraVoucherForm">
            ...
            <div class="mb-3">
              ...
              <select class="form-control @error('debit_ledger_id') is-invalid @enderror" id="create_debit_ledger_id" name="debit_ledger_id">
                <option value="">Select Ledger</option>
                @foreach ($ledgers as $ledger)
                  <option value="{{ $ledger->id }}" {{ old('debit_ledger_id') == $ledger->id ? 'selected' : '' }}>{{ $ledger->name }}</option>
                @endforeach
              </select>
            ...
            <div class="mb-3">
              ...
              <select class="form-control @error('credit_ledger_id') is-invalid @enderror" id="create_credit_ledger_id" name="credit_ledger_id">
                <option value="">Select Ledger</option>
                @foreach ($ledgers as $ledger)
                  <option value="{{ $ledger->id }}" {{ old('credit_ledger_id') == $ledger->id ? 'selected' : '' }}>{{ $ledger->name }}</option>
                @endforeach
              </select>
  ...
  @foreach ($vouchers as $voucher)
    <div class="modal fade" id="editContraVoucherModal{{ $voucher->id }}" tabindex="-1" aria-labelledby="editContraVoucherModalLabel{{ $voucher->id }}" aria-hidden="true">
      <div class="modal-dialog">
        <div class="modal-content">
          <div class="modal-header">
            <h5 class="modal-title" id="editContraVoucherModalLabel{{ $voucher->id }}">Edit Contra Voucher</h5>
            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
          </div>
          <div class="modal-body">
            <form action="{{ route('vouchers.contra.update', $voucher->id) }}" method="POST" id="editContraVoucherForm{{ $voucher->id }}">
              @csrf
              @method('PUT')
              <div class="mb-3">
              ...
              <div class="mb-3">
                <label for="edit_debit_ledger_id_{{ $voucher->id }}" class="form-label">Debit Ledger</label>
                <select class="form-control @error('debit_ledger_id') is-invalid @enderror" id="edit_debit_ledger_id_{{ $voucher->id }}" name="debit_ledger_id">
                  <option value="">Select Ledger</option>
                  @foreach ($ledgers as $ledger)
                    <option value="{{ $ledger->id }}" {{ old('debit_ledger_id', $voucher->entries->where('type', 'debit')->first()->ledger_id) == $ledger->id ? 'selected' : '' }}>{{ $ledger->name }}</option>
                  @endforeach
                </select>
                @error('debit_ledger_id')
                  <div class="invalid-feedback">{{ $message }}</div>
              ...
              <div class="mb-3">
                <label for="edit_credit_ledger_id_{{ $voucher->id }}" class="form-label">Credit Ledger</label>
                <select class="form-control @error('credit_ledger_id') is-invalid @enderror" id="edit_credit_ledger_id_{{ $voucher->id }}" name="credit_ledger_id">
                  <option value="">Select Ledger</option>
                  @foreach ($ledgers as $ledger)
                    <option value="{{ $ledger->id }}" {{ old('credit_ledger_id', $voucher->entries->where('type', 'credit')->first()->ledger_id) == $ledger->id ? 'selected' : '' }}>{{ $ledger->name }}</option>
                  @endforeach
                </select>
                @error('credit_ledger_id')
                  <div class="invalid-feedback">{{ $message }}</div>
          ...
          <div class="modal-footer">
            ...
            <button type="submit" form="editContraVoucherForm{{ $voucher->id }}" class="btn btn-alt-primary">
