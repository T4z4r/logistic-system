  ...
  <div class="content1 p-2 rounded-0">
    <div class="block block-rounded shadow-sm py-5 rounded-0">
      ...
      <div class="mb-4 text-center">
        <button type="button" class="btn btn-alt-primary px-4 py-2" data-bs-toggle="modal" data-bs-target="#createGroupModal">
          ...
          <tbody>
            @forelse ($groups as $group)
              <tr>
                ...
                <td>
                  <button type="button" class="btn btn-sm btn-alt-primary" data-bs-toggle="modal" data-bs-target="#editGroupModal{{ $group->id }}">
                    <i class="fa fa-edit"></i> Edit
                  </button>
                  <form action="{{ route('chart.groups.destroy', $group->id) }}" method="POST" style="display:inline;">
                    @csrf
                    @method('DELETE')
        ...
        <div class="modal-body">
          <form action="{{ route('chart.groups.store') }}" method="POST" id="createGroupForm">
            ...
            <div class="mb-3">
              ...
              <select class="form-control @error('type') is-invalid @enderror" id="create_type" name="type">
    ...
    <div class="modal fade" id="editGroupModal{{ $group->id }}" tabindex="-1" aria-labelledby="editGroupModalLabel{{ $group->id }}" aria-hidden="true">
      <div class="modal-dialog">
        <div class="modal-content">
          ...
          <div class="modal-body">
            <form action="{{ route('chart.groups.update', $group->id) }}" method="POST" id="editGroupForm{{ $group->id }}">
              @csrf
              @method('PUT')
              <div class="mb-3">
                <label for="edit_name_{{ $group->id }}" class="form-label">Name</label>
                <input type="text" class="form-control @error('name') is-invalid @enderror" id="edit_name_{{ $group->id }}" name="name" value="{{ old('name', $group->name) }}">
                @error('name')
                  <div class="invalid-feedback">{{ $message }}</div>
                @enderror
              </div>
              <div class="mb-3">
                ...
                <select class="form-control @error('parent_id') is-invalid @enderror" id="edit_parent_id_{{ $group->id }}" name="parent_id">
                  <option value="">None</option>
                  @foreach ($groups as $parent)
                    @if ($parent->id != $group->id)
                      <option value="{{ $parent->id }}" {{ old('parent_id', $group->parent_id) == $parent->id ? 'selected' : '' }}>{{ $parent->name }}</option>
                    @endif
                  @endforeach
                </select>
                @error('parent_id')
              ...
              <div class="mb-3">
                ...
                <select class="form-control @error('type') is-invalid @enderror" id="edit_type_{{ $group->id }}" name="type">
                  <option value="">Select Type</option>
                  @foreach (['asset', 'liability', 'income', 'expense'] as $type)
                    <option value="{{ $type }}" {{ old('type', $group->type) == $type ? 'selected' : '' }}>{{ ucfirst($type) }}</option>
                  @endforeach
                </select>
                @error('type')
                  <div class="invalid-feedback">{{ $message }}</div>
