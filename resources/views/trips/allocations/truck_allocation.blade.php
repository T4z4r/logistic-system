...
@section('content')
    ...
    <div class="card  border-top  border-top-width-3 border-top-black rounded-0 mt-5">
        ...
        <div class="card-body ">
            ...
            <div class="">
                <h4 class="lead  "> <i class="ph-truck text-brand-secondary"></i><code>{{ $allocation->ref_no }}</code>
                    Allocation
                    Details</h4>
                ...
                @if ($level && $allocation->status > 0 && $allocation->status != 4)
                    @if ($level->level_name == $allocation->approval_status)
                        ...
                        <div class="d-none d-sm-block m-2  col-12">
                            ...
                            <button class="btn btn-sm btn-alt-danger edit-button" title="Dispprove Allocation"
                        ...
                        <div class="d-sm-none   col-12">
                            ...
                            <div id="approveDiv" class="collapse col-12 mb-2">
                                <form action="{{ url('trips/approve-allocation') }}" id="change_route_form1"
                                    method="POST">
                                    @csrf
                                    <div class="row">
                                    ...
                                    <button type="submit" class="btn btn-sm btn-alt-primary "> <i class="fa fa-check"></i>
                                        Submit Approval
                                    </button>
                                </form>
                            </div>
                            <button type="button" class="btn btn-alt-danger mx-1 btn-sm" data-bs-toggle="collapse"
                                data-bs-target="#disapproveDiv">
                                <i class="fa fa-times"></i>
                            ...
                            <div id="disapproveDiv" class="collapse col-12  mb-2">
                                ...
                                <form action="{{ url('trips/disapprove-allocation') }}" id="change_route_form1"
                                    ...
                                    <button type="submit" class="btn btn-sm btn-alt-primary"> <i
                                            class="ph-check-circle"></i> Submit Disapproval</button>
                ...
                @if ($trip)
                    ...
                    @if ($trip->status == -1)
                        @can('initiate-trip')
                            <a href="javascript:void(0)" title="Revoke Trip"
                                class="icon-2 info-tooltip btn btn-alt-danger btn-sm mx-1"
                    ...
                    <a href="
                        ...
                        {{ url('/trips/backload-trip/' . base64_encode($allocation->id)) }} @endif
                         "
                        class="btn btn-alt-primary btn-sm float-end">
                        <i class="ph-list"></i>
                        View Trip Details
                    </a>
                @endif
            ...
            <div class="col-12 p-2 my-1 text-decoration-none remark bg-light " id="remark">
                @php
                    $remarks = App\Models\AllocationRemark::where('allocation_id', $allocation->id)->latest()->get();
                @endphp
                @if ($remarks->count() > 0)
            ...
            <div class="col-12 mx-auto">
                ...
                {{-- Allocation Details Table View --}}
                <div class="row  " id="hiddenDiv1">
                    <h5><span class="badge bg-primary text-light">Current To:</span> {{ $current_person }}</h5>
                    <hr>
                        ...
                        <table class="table table-bordered table-sm">
                            <tbody>
                                <tr>
                                    ...
                                    <td>{{ $allocation->customer?->contact_person ?? '--' }}</td>
                                    <th>Company Name</th>
                                    <td>{{ $allocation->customer?->company ?? '--' }}</td>
                                </tr>
                                <tr>
                                    <th>Email</th>
                                    <td>{{ $allocation->customer?->email ?? '--' }}</td>
                                    <th>Phone Number</th>
                                    <td>{{ $allocation->customer?->phone ?? '--' }}</td>
                                </tr>
                                <tr>
                                    <th>Address</th>
                                    <td>{{ $allocation->customer?->address ?? '--' }}</td>
                                    <th>TIN</th>
                                    <td>{{ $allocation->customer?->TIN ?? '--' }}</td>
                                </tr>
                                <tr>
                                    <th>VRN</th>
                                    <td>{{ $allocation->customer?->VRN ?? '--' }}</td>
                                    <th>Created Date</th>
                                ...
                                <tr>
                                    <th>Loading Point</th>
                                    <td>{{ $allocation->loading_site }}</td>
                                    <th>Offloading Point</th>
                                    <td>{{ $allocation->offloading_site }}</td>
                                </tr>
                                <tr>
                                    <th>Clearance</th>
                                    <td colspan="3">{{ $allocation->clearance }}</td>
                                </tr>
                            </tbody>
                        </table>
                    </div>
                    ...
                    <div class="col-12 mt-4">
                        <h5 class="text-muted">Trip Details</h5>
                        <table class="table table-bordered table-sm">
                            <tbody>
                                <tr>
                                    <th>Booked Route</th>
                                ...
                                <tr>
                                    <th>Cargo Nature</th>
                                    <td>{{ $allocation->nature?->name ?? '--' }}</td>
                                    <th>Cargo Dimensions</th>
                                    <td>{{ $allocation->dimensions }}</td>
                                </tr>
                                <tr>
                                    <th>Container</th>
                                    <td>{{ $allocation->container }}</td>
                                    <th>Container Type</th>
                                    <td>{{ $allocation->container == 'Yes' ? $allocation->container_type : '--' }}</td>
                                </tr>
                                <tr>
                                ...
                                <tr>
                                    ...
                                    <td colspan="3">
                                        <small class="text-danger">(Based on Estimated Quantity)</small><br>
                                ...
                                <tr>
                                    <th>Selected Trucks Revenue</th>
                                    <td colspan="3">
                                        <small class="text-danger">(Based on Selected Truck)</small><br>
                                        @php
                                            $truck_income = App\Models\TruckAllocation::where(
                                                'allocation_id',
                                                $allocation->id,
                                            )->sum('usd_income');
                                        @endphp
                                        {{ $allocation->currency->symbol }} {{ number_format($truck_income, 2) }}
                                    </td>
                                </tr>
                                <tr>
                                    <th>Total Cost</th>
                                    <td colspan="3">
                                        @php
                                            $truck_cost = 0;
                                            $trucks_allocated = App\Models\TruckAllocation::where(
                                                'allocation_id',
                                                $allocation->id,
                                            )->get();
                                            $semi = 0;
                                            ...
                                            foreach ($trucks_allocated as $truc) {
                                                ...
                                                $truck_cost += App\Models\TruckCost::where(
                                                    'allocation_id',
                                                    $truc->id,
                                                )->sum('real_amount');
                                            }

                                            $total_allocated_trucks = $trucks_allocated->count();
                                            $total_costs =
                                            ...
                                            $total_income = App\Models\TruckAllocation::where(
                                                'allocation_id',
                                                $allocation->id,
                                            )->sum('income');
                                            $profit =
                                                ($total_income - $total_summed_cost) / $allocation->currency->rate;
                                        @endphp
                                        {{ $allocation->currency->symbol }} {{ number_format($total_summed_cost, 2) }}
                                    </td>
                                </tr>
                                <tr>
                                    <th>Estimated Profit</th>
                                    <td colspan="3">{{ $allocation->currency->symbol }}
            ...
            <div class="">
                ...
                {{-- @can('edit-allocation') --}}
                @if ($allocation->status <= 0)
                    <button class="btn btn-alt-primary btn-sm edit-button float-end" data-bs-toggle="modal"
                    ...
                    {{-- For Change Route --}}
                    <button type="button" class="btn btn-alt-primary mx-1 btn-sm float-end" data-bs-toggle="collapse"
                    ...
                    <div id="myCollapsibleDiv" class="collapse">
                        ...
                        <br>
                        <hr>
                        <form action="{{ route('flex.change-allocation-route') }}" id="change_route_form"
                            method="POST">
                            @csrf
                            <div class="row">
                                ...
                                <div class="col-md-6 col-lg-6 mb-1">
                                    ...
                                    <select name="old_route" class="select form-control">
                                        <option value="{{ $allocation->route_id }}">{{ $allocation->route->name }}
                                        </option>
                                    </select>
                                </div>
                                <div class="col-md-6 col-lg-6 mb-1">
                            ...
                            <button type="submit" class="btn btn-sm btn-alt-primary float-end" id="change_route_btn"> <i
                                    class="ph-recycle"></i> Change</button>
                            <br>
                            <br>
                            <hr>
                        </form>
                ...
                <div class="tab-content  ">
                                ...
                                <table
                                    ...
                                    <tbody>
                                        ...
                                        @forelse($trucks as $item)
                                            @php
                                                ...
                                                $drivers = App\Models\DriverAssignment::where(
                                                    'truck_id',
                                                    $item->id,
                                                )->first();
                                                $allocated = App\Models\TruckAllocation::where('truck_id', $item->id)
                                                    ...
                                                    ->where('status', 0)
                                                    ->first();
                                                $manager = App\Models\TruckManagerAssignment::where(
                                                    'truck_id',
                                                    $item->id,
                                                )
                                                    ->latest()
                                                    ->first();
                                                $capacity = 0;
                                                if ($trailers && $trailers->count() > 0) {
                                                    foreach ($trailers as $item2) {
                                                        if ($item2->trailer) {
                                                            $capacity += $item2->trailer?->capacity ?? 0;
                                                        }
                                                    }
                                                }
                                            @endphp
                                            @if ($trailers->count() > 0 && $drivers && $allocated == null)
                                                <tr>
                                    ...
                                    margin:auto;"
                                                    ...
                                                    <td style="width:30% !important">
                                                        {{ $drivers->driver->name ?? 'Not Assigned' }}
                                        ...
                                        <div class="form-group">
                                            @if ($remaining > 0)
                                                <button type="submit" id="add_trucks_btn"
                                                    class="btn btn-sm btn-alt-success float-end">
                                                    Add Selected Trucks
                                                </button>
                                            @endif
                    ...
                    <div class="tab-pane fade  show" id="selected" role="tabpanel">
                        ...
                        <form action="{{ route('flex.remove_bulk_trucks') }}" id="remove_trucks_form" method="post">
                            ...
                            <hr>
                            @php
                                $planned = App\Models\TruckAllocation::where('allocation_id', $allocation->id)->sum(
                                    'planned',
                                );
                                $remaining = $allocation->quantity - $planned;
                            @endphp
                                    ...
                                    @forelse($trucks as $item)
                                        ...
                                        <tr>
                                            ...
                                            @if ($allocation->type == 2)
                                                <td>
                                                    <span
                                                        class="badge {{ $item->mobilization == '1' ? ' bg-success ' : ' bg-info ' }}  bg-opacity-10 ">
                                            ...
                                            <td style="">
                                                ...
                                                {{-- <a class="btn btn-alt-primary btn-sm add-plan" data-bs-toggle="modal"
                                                ...
                                                @if ($item->mobilization == 1)
                                                    ...
                                                    <a href="{{ url('/trips/demobilize-truck/' . base64_encode($item->id)) }}"
                                                        title="Add Truck Cost" class="btn btn-sm btn-alt-danger"
                                                        id="demobilize">
                            ...
                            <div class="row mb-1">
                                ...
                                <div class="col-12">
                                    ...
                                    <button type="submit" id="remove_trucks_btn"
                                        class="btn btn-sm btn-alt-danger float-end mx-2">
                                        Remove Selected Trucks
                                    </button>
                                    {{-- @endcan --}}
                                    @if ($allocation->type == 2)
                                        <button type="button"
                                            class="btn btn-alt-primary btn-sm mobilize-button float-end mx-2"
                                        ...
                                        <button type="button"
                                            class="btn btn-alt-danger btn-sm mobilize-button float-end mx-2"
    ...
    @else
                ...
                <tbody>
                    ...
                    @forelse($trucks as $item)
                        <tr>
                            ...
                            <td>
                                <a href="{{ url('/trips/truck-cost/' . base64_encode($item->id)) }}"
                                    title="View Truck Cost" class="btn btn-sm btn-alt-primary">
                                    <i class="fa fa-list"></i>
                                </a>
                                <br>
                                <br>
                                ...
                                @can('replace-truck')
                                    {{-- <a href="{{ url('trips/remove-truck/' . $item->id) }}"
                                            class="btn btn-sm btn-alt-danger m-1"> Remove </a>

                                            <br> --}}
                                    {{-- <button type="button" class="btn btn-sm btn-alt-primary replace" data-bs-toggle="modal" data-bs-target="#replaceModal"
        ...
        <div class="p-2">
            ...
            @if ($allocation->status <= 0)
                {{-- @can('add-trip-cost') --}}
                <button class="btn btn-sm btn-alt-primary float-end mb-3" data-bs-toggle="modal"
                                        ...
                                        <td>
                                            @if ($item->editable == 1)
                                                ...
                                                <a href="javascript:void(0)" title="Remove Cost"
                                                    class="icon-2 info-tooltip btn btn-alt-danger btn-sm {{ $item->status == '0' ? '' : 'disabled' }} "
        ...
        <div class="modal-dialog" role="document">
            ...
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title "><i class="fa fa-edit text-brand-secondary"></i> Edit Allocation</h5>
                    <button type="button" class="btn-close btn-alt-danger" data-bs-dismiss="modal">
                    </button>
                </div>
                <form action="{{ route('flex.update-allocation') }}" id="update_allocation_form" method="POST"
                    ...
                    <div class="modal-footer">
                        ...
                        <button type="submit" id="update_allocation_btn" class="btn btn-alt-primary btn-sm"> Update
                            Request &nbsp; <i class="fa fa-truck"></i></button>
                        </a>
                    </div>
                </form>
            </div>
        ...
        <div class="">
            ...
            <div class="modal fade modal-xl" id="mobilization-modal" tabindex="-1" role="dialog"
                ...
                <div class="modal-dialog modal-dialog-centered" role="document">
                    ...
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title "><i class="fa fa-truck text-brand-secondary"></i> Mobilization Costs
                            </h5>
                            <button type="button" class="btn-close btn-alt-danger" data-bs-dismiss="modal">
                            </button>
                        </div>
                        <form action="{{ route('flex.mobilize_bulk_trucks') }}" id="update_allocation_form"
                                ...
                                <div class="col-md-6 col-12 mb-2 float-end">
                                    ...
                                    <div class="form-group">
                                        <button type="submit" id="pay_allocation_btn"
                                            class="btn btn-sm btn-alt-primary float-end mb-2">
                                            <i class="fa fa-check"></i>
                            ...
                            <div class="modal-footer">
                                ...
                                {{-- <button type="submit" id="update_allocation_btn" class="btn btn-alt-primary btn-sm"> Mobilize Trucks &nbsp; <i class="ph-truck"></i></button> --}}
                                </a>
                            </div>
                        </form>
                    </div>
        ...
        <div class="">
            ...
            <div class="modal fade modal-xl" id="demobilization-modal" tabindex="-1" role="dialog"
                ...
                <div class="modal-dialog modal-dialog-centered" role="document">
                    ...
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title "><i class="fa fa-truck text-brand-secondary"></i> Demobilize Trucks
                            </h5>
                            <button type="button" class="btn-close btn-alt-danger" data-bs-dismiss="modal">
                            </button>
                        </div>
                ...
                <div class="modal-dialog modal-dialog-centered ">
                    <div class="modal-content">
                        <div class="modal-header">
                            ...
                            <button type="button" class=" btn-alt-danger btn-close" data-bs-dismiss="modal"
                                    ...
                                    <select name="truck_id" class="select form-control">
                                        ...
                                        @forelse($trucks as $item)
                                            ...
                                            @if ($trailers->count() > 0 && $drivers)
                                                <option value="{{ $item->id }}">{{ $item->plate_number }} -
                        ...
                        <div class="modal-footer">
                            <button type="button" class="btn btn-alt-danger" data-bs-dismiss="modal">Cancel</button>
                            <button type="submit" id="confirm_replace_btn" class="btn btn-main">Confirm Replace</button>
                        </div>
                        </form>
                        {{-- <a href="{{ url('trips/remove-truck/' . $item->id) }}" class="btn btn-alt-danger">Confirm Replace</a> --}}
                        {{-- </div> --}}
                        {{-- </form> --}}
                ...
                <div class="modal-content">
                    <form id="edit-form" method="POST" action="{{ route('flex.update-allocation-cost') }}">
                        ...
                        <div class="modal-header">
                            ...
                            <button type="button" class="close btn-alt-danger text-light" data-bs-dismiss="modal"
                                aria-label="Close">
                            </button>
                        </div>
                        <div class="modal-body">
    ...
    <div id="disapproval" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="disapproval-modal-label">
                        ...
                        <div class="row ">
                            <div class="col-sm-4 col-8 mx-auto">
                                ...
                                <button type="button" id="disapprove_no"
                                    class="btn btn-alt-danger btn-sm  px-4 text-light" data-bs-dismiss="modal">
                                    No
                                </button>
                            </div>
                        </div>
    ...
    <div id="approval" class="modal fade" {{-- tabindex="-1" --}} tabindex="-1" role="dialog"
                        ...
                        <div class="row">
                            <div class="col-sm-4 col-8 mx-auto d-flex justify-content-between">
                                <button type="submit" id="approve_yes"
                                    class="btn btn-alt-primary btn-sm px-4 mx-1">Yes</button>

                                <button type="button" id="approve_no" class="btn btn-alt-danger btn-sm px-4 text-light"
        ...
        <script src="https://code.jquery.com/jquery-3.7.1.min.js"
            integrity="sha256-3gJwYp4SXN9KejGAAZ8nQhC7LsLhH9V3IrM3x+p5DPE=" crossorigin="anonymous"></script>

        <!-- Bootstrap 5 JS Bundle with Popper -->
        <script src="https://cdn.jsdelivr.net/npm/bootstrap@5.3.3/dist/js/bootstrap.bundle.min.js"
            integrity="sha384-w76A2z02tPqdjP3WA2bFZ8t7ZtDfN1zLxgL4TcN4RAfK6oE6tM9CkDQ3CKP9xjYg" crossorigin="anonymous">
        </script>
