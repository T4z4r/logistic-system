        ...
        <form action="{{ route('drivers.store') }}" method="POST">
          ...
          <div class="row">
            <div class="col-md-6 mb-4">
              ...
              <input type="text" name="name" id="name" placeholder="Enter driver name" class="form-control @error('name') is-invalid @enderror" value="{{ old('name') }}">
              @error('name')
            ...
            @foreach ($managers as $manager)
              <option value="{{ $manager->id }}" {{ old('created_by') == $manager->id ? 'selected' : '' }}>{{ $manager->name }}</option>
            @endforeach
              </select>
              @error('created_by')
            <div class="invalid-feedback">{{ $message }}</div>
