    ...
    <div class="content content-full">
      <div class="d-flex flex-column flex-sm-row justify-content-sm-between align-items-sm-center py-2">
        <div class="flex-grow-1">
          <h1 class="h3 fw-bold mb-1">Create Trailer Assignment</h1>
          <h2 class="fs-base lh-base fw-medium text-muted mb-0">Assign a trailer to a truck</h2>
        </div>
  ...
  <div class="content">
    ...
    <div class="block block-rounded">
      <div class="block-header block-header-default">
        <h3 class="block-title">New Trailer Assignment Form</h3>
      </div>
      <div class="block-content">
        <form action="{{ route('trailer-assignments.store') }}" method="POST">
          @csrf
          <div class="mb-4">
            ...
            <select name="trailer_id" id="trailer_id" class="form-control @error('trailer_id') is-invalid @enderror">
              ...
              @foreach ($trailers as $trailer)
                <option value="{{ $trailer->id }}" {{ old('trailer_id') == $trailer->id ? 'selected' : '' }}>{{ $trailer->plate_number }}</option>
              @endforeach
            </select>
            @error('trailer_id')
